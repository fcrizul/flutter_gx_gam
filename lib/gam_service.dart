...
class GAMService {
  static Future<GAMLoginResponse> login(
    ...
    try {
      ...
      if (ValidatorsHelper.isJson(responseBody)) {
        ...
        if (response.statusCode == 200 && responseMap["access_token"] != null) {
          ...
          _debug('Valor de access_token en sesion: ' +
          ...
          if (responseMap.containsKey('error')) {
            ...
            return GAMLoginResponse(
                errorCode: int.parse(error["code"]),
                errorMessage: error["message"]);
          } else {
            return GAMLoginResponse(
                errorCode: 700, errorMessage: responseMap['object'].toString());
          }
        }
      } else {
        return GAMLoginResponse(
            errorCode: response.statusCode, errorMessage: responseBody);
      }
    } on TimeoutException catch (_) {
      return GAMLoginResponse(
          errorCode: 1, errorMessage: "Error TimeoutException");
    } on SocketException catch (_) {
      return GAMLoginResponse(
          errorCode: 2, errorMessage: "Error SocketException");
  ...
  static Future<bool> isAuthorized(String permission) async {
    assert(GAMConfig.checkPermission != null,
        "GAMConfig().checkPermission is null");
